...
export function MobileCeoPeriodChips({ from, to, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 flex gap-1.5 overflow-x-auto no-scrollbar -mx-1 px-1 pb-1">
        {PRESETS.map((p) => (
          <button
            key={p.key}
            onClick={() => handlePreset(p.key)}
            className={cn(
              'shrink-0 h-9 px-3.5 rounded-full text-sm font-medium transition-all',
              active === p.key
        ...
        <Sheet open={openSheet} onOpenChange={setOpenSheet}>
          <SheetTrigger asChild>
            <button
              className={cn(
                'shrink-0 h-9 px-3 rounded-full text-sm font-medium flex items-center gap-1.5 transition-all',
                active === 'custom'
                  ? 'bg-foreground text-background shadow-sm'
                  : 'bg-muted/60 text-foreground/70 hover:bg-muted'
              )}
            >
              ...
              {active === 'custom' ? periodLabel : 'Custom'}
            </button>
          </SheetTrigger>
