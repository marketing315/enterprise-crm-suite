...
| **F5** Rifiniture | 🟡 In corso | ✅ 1-4 (MV channel/salesperson daily + cron 15min, badge freshness, A/B compare marketing, per-fonte sales, drill-down venditore con funnel+trend). ✅ 5 alert performance (regole soglie CPL/answer-rate/deliveries/sentiment + RPC `evaluate_performance_alerts` + edge `performance-alerts-evaluator` cron 30min + mirror su `mcp_slo_alerts` + UI `/admin/performance-alerts`). ⏳ 6-7 export Sheets + DPIA |
