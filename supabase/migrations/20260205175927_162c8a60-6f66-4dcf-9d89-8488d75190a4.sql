-- Drop existing functions first to update signatures
DROP FUNCTION IF EXISTS public.search_tickets_v2(UUID, TEXT, UUID, TEXT, UUID[], TEXT, TEXT[], TEXT, INT, JSONB, TEXT);
DROP FUNCTION IF EXISTS public.get_ticket_queue_counts(UUID, UUID, TEXT, TEXT, UUID[]);

-- Updated search_tickets_v2 with p_brand_ids support for cross-brand view
...
BEGIN
  ...
  WITH filtered_tickets AS (
    ...
    WHERE t.brand_id = ANY(v_effective_brand_ids)
      ...
      -- Status filter
      AND (p_statuses IS NULL OR t.status = ANY(p_statuses::ticket_status[]))
  ),
  counted AS (
...
-- Updated get_ticket_queue_counts with p_brand_ids support
