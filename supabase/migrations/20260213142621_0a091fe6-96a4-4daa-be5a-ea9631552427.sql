...
-- 1. Set platform on existing channels
UPDATE marketing_channels SET platform = 'meta' WHERE id = 'c1000000-0000-0000-0000-000000000001';
UPDATE marketing_channels SET platform = 'google' WHERE id = 'c1000000-0000-0000-0000-000000000002';

-- 2. Rewrite get_marketing_channel_kpis to use ad_platform_stats spend + source_name revenue
...
BEGIN
  ...
  -- Revenue via source_name: match lead source to channel name
  channel_revenue AS (
    SELECT
      ch.id as channel_id,
  ...
  -- Spend from ad_platform_stats (real API data)
  channel_adv_spend AS (
    SELECT
      ch.id as channel_id,
  ...
  LEFT JOIN channel_adv_spend cas ON cas.channel_id = ch.id
  WHERE (v_is_company_brand OR ch.brand_id = p_brand_id)
    AND ch.is_active = true
...
-- 3. Rewrite get_marketing_campaign_kpis to use ad_platform_stats spend + source_name revenue
CREATE OR REPLACE FUNCTION public.get_marketing_campaign_kpis(p_brand_id uuid, p_from date, p_to date, p_channel_id uuid DEFAULT NULL::uuid, p_campaign_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(campaign_id uuid, campaign_name text, channel_name text, leads_count bigint, deals_count bigint, deals_won bigint, revenue numeric, marketing_cost numeric, cpl numeric, cac numeric, roi numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  -- Revenue via source_name: join lead→contact→deal
  campaign_revenue AS (
    SELECT
      mcp.id as campaign_id,
  ...
  -- Spend from ad_platform_stats matched via campaign external_id
  campaign_adv_spend AS (
    ...
    JOIN ad_platform_stats aps ON (
      mcp.external_id IS NOT NULL AND aps.external_campaign_id = mcp.external_id
    )
      AND aps.stat_date >= p_from AND aps.stat_date <= p_to
      AND (v_is_company_brand OR aps.brand_id = p_brand_id)
    WHERE (v_is_company_brand OR mcp.brand_id = p_brand_id)
  ...
  LEFT JOIN campaign_revenue cr ON cr.campaign_id = mcp.id
  LEFT JOIN campaign_adv_spend cas ON cas.campaign_id = mcp.id
  WHERE (v_is_company_brand OR mcp.brand_id = p_brand_id)
    AND (p_channel_id IS NULL OR mcp.channel_id = p_channel_id)
    AND (p_campaign_id IS NULL OR mcp.id = p_campaign_id)
  ORDER BY COALESCE(cr.total_revenue, 0) DESC;
