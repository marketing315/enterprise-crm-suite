...
BEGIN
  -- 1. Ingest Availability (uses created_at, status)
  SELECT
    COUNT(*),
  ...
  -- 4. Webhook Delivery Rate (uses status + dead_at)
  SELECT
    COUNT(*),
  ...
  -- 5. Deal Velocity (uses closed_at + status='won' mapped from deals.status)
  SELECT
    COUNT(*),
  ...
  WHERE c.created_at >= p_month_start AND c.created_at < v_month_end
    AND d.status NOT IN ('lost', 'disqualified')
