...
-- RPC: Board-level SLO metrics (monthly)
-- Returns SLO health for each domain with ownership info
...
BEGIN
  -- 1. Ingest Availability (Engineering)
  SELECT
    COUNT(*),
    COUNT(*) FILTER (WHERE status != 'failed')
  INTO v_ingest_total, v_ingest_success
  FROM incoming_requests
  WHERE received_at >= p_month_start AND received_at < v_month_end
  ...
  -- 2. Ticket SLA Compliance (CX Ops)
  SELECT
    COUNT(*),
  ...
  -- 4. Webhook Delivery Rate (Engineering)
  SELECT
    COUNT(*),
  ...
  FROM deals
  WHERE closed_at IS NOT NULL AND stage_name = 'won'
    AND created_at >= p_month_start AND created_at < v_month_end
    AND (p_brand_id IS NULL OR brand_id = p_brand_id);

  -- 6. Lead → Deal Conversion (Sales Ops)
