...
-- Aggregated KPI summary function
CREATE OR REPLACE FUNCTION get_marketing_summary_kpis(
  p_brand_id uuid,
  p_from date,
  p_to date
)
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN COALESCE(a.total_leads, 0) > 0 
      THEN ROUND(COALESCE(a.total_cost, 0) / a.total_leads, 2)
      ELSE 0 
    END as avg_cpl,
    CASE WHEN COALESCE(a.total_deals_won, 0) > 0 
      THEN ROUND(COALESCE(a.total_cost, 0) / a.total_deals_won, 2)
      ELSE 0 
    END as avg_cac,
    CASE WHEN COALESCE(a.total_cost, 0) > 0 
      THEN ROUND((COALESCE(a.total_revenue, 0) - COALESCE(a.total_cost, 0)) / a.total_cost * 100, 2)
      ELSE 0 
    END as overall_roi
  FROM aggregated a;
END;
