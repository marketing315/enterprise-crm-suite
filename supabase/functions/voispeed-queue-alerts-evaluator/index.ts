// F6 Step #7 — VoiSpeed Queue Alerts Evaluator
// ─────────────────────────────────────────────────────────────────────────────
// Itera tutti i brand con almeno una regola attiva e chiama
// evaluate_voispeed_queue_alerts(brand). Per ogni evento creato ne fa anche
// mirror su mcp_slo_alerts per /admin/observability (stesso pattern di
// performance-alerts-evaluator). Le notifiche per i responsabili vengono
// inserite direttamente dalla RPC.
//
// Schedule: ogni minuto via pg_cron.
  ...
  const log = (level: "log" | "error", msg: string, extra?: Record<string, unknown>) =>
    console[level](
      JSON.stringify({
        ...
        fn: "voispeed-queue-alerts-evaluator",
        level,
        msg,
        ...extra,
      }),
  ...
  try {
    ...
    for (const brandId of uniqueBrands) {
      ...
      for (const ev of rows) {
        ...
        const windowStart = new Date(windowEnd.getTime() - 10 * 60 * 1000);

        const { error: insErr } = await supabase.from("mcp_slo_alerts").insert({
          alert_type: `voispeed_queue:${ev.metric}`,
          severity: sev,
          ...
          details: {
            source: "voispeed-queue-alerts-evaluator",
            brand_id: brandId,
            rule_id: ev.rule_id,
