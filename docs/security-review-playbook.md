...
- **H8 — Double-submit guard.** Ogni form che chiama una RPC mutativa DEVE avere `submitInFlightRef` (useRef) **+ UNIQUE constraint DB-side**. Pattern fragile da cercare in PR: `useState.*[Ss]ubmitting` o `disabled.*isSubmitting` **senza** ref accompagnato. **Audit Q2 2026 (chiuso)**: pattern ora applicato in `NewContactDialog`, `QuickSaleDialog`, `NewAppointmentDialog`, `CreateTicketDialog`, `Step2InviteUsers`. I form basati su `react-hook-form` (`WebhookFormDrawer`, `InboundSourceFormDrawer`) sono coperti dal lock interno di `handleSubmit()` ma è comunque consigliato il ref se la mutation tocca tabelle senza UNIQUE.
