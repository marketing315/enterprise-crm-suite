...
- *Note:* Creato `src/components/tickets/mobile/MobileTicketsList.tsx`: header sticky (titolo "Ticket" + counter totale) + `Segmented asTabs` con code (`all`/`my_queue`/`unassigned`/`sla_breached`); chip `my_queue` mostrato solo a operatori (admin/operatore_callcenter/responsabile_callcenter). Counter per coda dal `useTicketQueueCounts`. Default tab = `my_queue` per operatori, `all` altrimenti. Riusa `useTicketsSearch({queueTab, limit:100})` → stesso queryKey desktop, cache condivisa, zero RPC nuove. Lista `MobileListItem` con leading icon ticket (chip muted), title titolo ticket, subtitle `contatto · assegnatario · "aperto X" (formatDistanceToNow it)`, trailing stack verticale `TicketStatusBadge` + `TicketPriorityBadge` (riuso badge esistenti). Tap → `TicketDetailSheet` (riusa desktop). **Swipe actions context-aware**: ticket aperti (`open`/`in_progress`/`reopened`) mostrano "Prendi" (primary, icon UserCheck) — solo se utente è operatore e non già assegnatario — invoca `useAssignTicket` (gestione `STALE_TICKET` → toast "Ticket aggiornato altrove…"); + "Risolvi" (primary, icon CheckCircle2, `confirm` AlertDialog) → `useUpdateTicketStatus({status:"resolved"})`. Ticket chiusi/risolti senza action (read-only). FAB `MobileFab` (Plus, label "Nuovo ticket (scegli contatto)") → `navigate("/contacts")` perché `CreateTicketDialog` richiede `contactId` obbligatorio. `PullToRefresh` invalida `["tickets"]` + `["ticket-queue-counts"]`. Stati: `MobileListSkeleton count=6`, `ErrorState` retry, `EmptyState` differenziato per coda. Wired in `src/pages/Tickets.tsx`: `Tickets()` ora fa solo `useIsMobile()` + early return `<MobileTicketsList />`; tutta la logica desktop estratta in `TicketsDesktop()` interno (hooks order safe, zero modifiche funzionali). Solo token semantici. Mobile suite 124/124 verde. Follow-up: F4.5 (Lead in arrivo mobile).


### F4.5 — Lead in arrivo mobile `[x]`
- **dep:** F1.6, F2.4
- Lista cronologica + filtro canale; swipe Assegna/Chiama/Scarta; realtime se presente.
- **AC:** SPEC §6.7; stati gestiti.
- *Note:* Creato `src/components/leads/mobile/MobileLeadsInbox.tsx`: header sticky con totale lead, `Segmented<PeriodFilter>` (Oggi/7g/30g/Tutti) + `Segmented<source>` dinamico dai `source_name`, toggle "Mostra archiviati" admin/CEO. Lista cronologica `MobileListItem` con leading icona Inbox, badge fonte + `P{ai_priority}` (destructive≥8/default≥5/secondary), telefono mono, timestamp relativo+assoluto, tag (max 3). Swipe: `Assegna` (apre `ContactDetailSheet` se `contact_id`), `Chiama` (`tel:` se `primary_phone`), `Scarta` (variant `destructive` con `confirm`, RPC `set_lead_event_archived`); su archiviati solo `Ripristina`. Realtime: channel `mobile-leads-inbox` su `lead_events` invalida `lead-events-rpc`. `PullToRefresh` + `EmptyState`/`ErrorState`/`MobileListSkeleton`. `src/pages/Events.tsx` ora delega via `useIsMobile()` early-return; desktop estratto in `EventsDesktop()` (ordine hook safe, zero modifiche funzionali). Test 236/237 (1 failure pre-esistente su `auth-context` non correlato a F4.5).
...
### F5.2 — Performance venditori / Call center KPI mobile `[x]`
- **dep:** F5.1
- Stesso pattern; confronti come liste/grafici singoli.
- **AC:** SPEC §6.10.
- *Note:* Creati `src/components/sales/mobile/MobileSalesPerformanceSheet.tsx` e `src/components/admin/mobile/MobileCallcenterKpi.tsx` sullo stesso pattern di `MobilePerformanceHub`. **Sales sheet mobile**: header sticky + `Segmented<Period>` Mese/Scorso/30g; riusa `useSalespersonKpisV2`+`useSalespersonKpisAggregate` (zero RPC nuove, cache desktop condivisa); `HeroMetricCard` Lordo periodo (caption `N venditori · N ordini`); `KpiList` con `MetricRow` (Imponibile, Bonus totale, Appuntamenti eseguiti+% esecuzione, Consegnati periodo); lista venditori ordinata per lordo desc → card con iniziali (chip 40px muted) + nome + subtitle `N ord · esec X%` + tier badge + trailing `Lordo` (sm, semibold) + `Bonus` (xs muted) + chevron; tap → `navigate("/sales/performance-sheet/:user_id")`. `EmptyState`/`ErrorState`/`MobileListSkeleton`. `PullToRefresh` invalida `salesperson-kpis-v2`+`salesperson-kpis-aggregate`. Fallback `EmptyState` se no brand attivo. **Callcenter KPI mobile**: header sticky + `Segmented<PeriodKey>` 7g/30g/90g; riusa `useCallcenterKpisOverview`+`useCallcenterKpisByOperator` (zero RPC nuove); `HeroMetricCard` Ticket creati (variant `negative` se unassigned>0 || backlog>20, altrimenti `primary`); `KpiList` con `MetricRow` (Presi in carico+Avg assign, Risolti+Avg resolve, Backlog, Non assegnati con tone `warning` se >0); lista operatori ordinata per `tickets_resolved` desc → card con iniziali + nome + subtitle `Call Center/Sales · assegn. avg · risolto avg` + trailing risolti + backlog colorato (destructive>10 / warning>5 / muted); footer hint per telefonia desktop. `EmptyState`/`ErrorState`/`MobileListSkeleton`. `PullToRefresh` invalida entrambe le query. Wiring: `SalesPerformanceSheet.tsx` e `AdminCallcenterKpi.tsx` ora fanno `useIsMobile()` early-return e estraggono il desktop in `SalesPerformanceSheetDesktop`/`AdminCallcenterKpiDesktop` (zero modifiche funzionali). Solo token semantici. Follow-up: F5.3 (Wallboard call center mobile).

### F5.3 — Wallboard call center mobile `[x]`
- **dep:** F1.5, F2.4
- Numeri grandi impilati (code/attese/SL), auto-refresh esistente.
- **AC:** SPEC §6.11; leggibile a distanza.
- *Note:* Creato `src/components/callcenter/mobile/MobileCallcenterWallboard.tsx`. Header sticky con brand + stato live/manuale (badge `Activity` colorato), `Segmented<RangePreset>` Oggi/7g/30g + `Select` refresh (Off/15/30/60s) + bottone manual refresh (`RefreshCw` con `animate-spin` durante fetching). Riusa `useOperatorKpis(fromIso,toIso,refreshMs)` + `useTrackingNumberPerformance(...)` con stessa logica `computeRange` (window end = inizio giorno successivo) — zero RPC nuove, refresh interval propagato. **KPI hero**: `HeroMetricCard` "Chiamate nel periodo" con caption `Answer rate X% · AHT m m s` (variant `negative` se answerRate<70%, altrimenti `primary`). `KpiList` con `MetricRow` (In entrata, In uscita, Risposte, Perse con tone `warning` se >0, Talk time). **Canali inbound**: top-6 con card icon `Radio` 40px, nome canale, `N risposte · talk Xm Ys`, trailing `inbound count` (semibold) + `% risp` con tone `warning` se <70%. **Operatori**: ordinati per `calls_total` desc, card con iniziali, subtitle `inbound · outbound · talk`, trailing total + answer-rate colorato. `EmptyState` differenziati per nessuna inbound / nessun operatore / no brand selezionato (o azienda intera). `ErrorState` con retry sul KPI. `PullToRefresh` invoca `refetch()` su entrambe le query. Wiring: `CallcenterWallboard.tsx` ora fa `useIsMobile()` early-return e desktop estratto in `CallcenterWallboardDesktop` (zero modifiche funzionali, `LiveAgentsPanel` resta solo su desktop). Solo token semantici (`text-success`/`text-warning`/`bg-muted`). Follow-up: F5.4 (Vendite/Prodotti/Azienda mobile).

### F5.4 — Vendite / Prodotti / Azienda mobile `[x]`
- **dep:** F1.5, F1.6, F2.4
- Card+liste; tabelle→liste; KPI in hero.
- **AC:** SPEC §6.9; dati invariati.
- *Note:* Creati `src/components/sales/mobile/MobileSales.tsx`, `src/components/sales/mobile/MobileProducts.tsx`, `src/components/company/mobile/MobileCompanyOverview.tsx` sullo stesso pattern delle altre schermate F5. **Sales mobile** (`/sales`): header sticky con titolo+brand+CTA "Rapida" (apre `QuickSaleDialog`); `Segmented<Period>` Oggi/7g/Mese/Tutto; `HeroMetricCard` Fatturato 30g (caption `N ordini · valore medio`) + `KpiList` (Pagati+in attesa, Tasso conversione); ricerca per numero/cliente + chip orizzontale di stati ordine (`ORDER_STATUS_CONFIG`); lista ordini come card cliccabili (numero+badge stato, contatto+data, total+pagato, chevron) → apre `SalesOrderDetailSheet` desktop. Riusa `useSalesOrders`+`useSalesKpis` (zero RPC nuove). **Products mobile** (`/products`): header sticky con titolo+conteggio+CTA Nuovo; ricerca + toggle "Mostra inattivi"; lista prodotti come card (icona Package, nome+badge Inattivo, SKU+IVA, prezzo+pencil) → apre `Dialog` di modifica con form mobile-friendly (Disattiva, Annulla, Crea/Salva). Riusa `useProducts`/`useCreateProduct`/`useUpdateProduct`/`useDeleteProduct`. **Company Overview mobile** (`/azienda`): `Segmented<Period>` 1m/3m/6m/12m; `HeroMetricCard` Margine (variant `negative` se <0); `KpiList` (Vendite, Costi tone `warning`, Budget vs Actual con `positive`/`negative`, Tendenza margine); top-5 categorie di costo con % sul totale; lista ultimi 10 costi (categoria, data+fornitore+descrizione, importo). Riusa `useFinanceKpis`+`useExpenses`+`useHasFinanceAccess`. `EmptyState` per no brand / accesso negato / lista vuota; `ErrorState` con retry. `PullToRefresh` invalida `sales-orders`/`sales-kpis`, `products`, `finance-kpis`/`expenses`. Wiring: `Sales.tsx`, `Products.tsx`, `company/CompanyOverview.tsx` ora fanno `useIsMobile()` early-return; desktop estratto in `SalesDesktop`/`ProductsDesktop`/`CompanyOverviewDesktop` (zero modifiche funzionali). Solo token semantici (`bg-card`/`text-muted-foreground`/`bg-muted`). Follow-up: F6.1 (Notifiche mobile).
...
### F6.2 — Impostazioni / Profilo mobile `[x]`
- **dep:** F2.4
